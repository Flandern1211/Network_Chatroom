...
func (s *Server) AddUser(name string, c *ClientConn) bool {
	//基本校验：
	if name == "" || strings.Contains(name, " ") || len(name) > 32 {
		return false
	}
...
func (s *Server) RemoveUser(name string) error {
	...
	if _, ok := s.users[name]; ok {
		delete(s.users, name)
		fmt.Printf("删除用户%s成功", name)
		return nil
	}
	s.mu.Unlock()
	return fmt.Errorf("删除用户%s失败", name)
